...
function SelectTrigger({
	...
	return (
		<SelectPrimitive.Trigger
			...
			data-size={size}
			className={cn(
				'iso:flex iso:w-fit iso:items-center iso:justify-between iso:gap-2 iso:rounded-md iso:border iso:border-input iso:bg-background iso:px-3 iso:py-2 iso:text-sm iso:whitespace-nowrap iso:shadow-xs iso:transition-[color,box-shadow] iso:outline-none iso:focus-visible:border-ring iso:focus-visible:ring-[3px] iso:focus-visible:ring-ring/50 iso:disabled:cursor-not-allowed iso:disabled:opacity-50 iso:aria-invalid:border-destructive iso:aria-invalid:ring-destructive/20 iso:data-[placeholder]:text-muted-foreground iso:data-[size=default]:h-9 iso:data-[size=sm]:h-8 iso:*:data-[slot=select-value]:line-clamp-1 iso:*:data-[slot=select-value]:flex iso:*:data-[slot=select-value]:items-center iso:*:data-[slot=select-value]:gap-2 iso:dark:bg-input/30 iso:dark:hover:bg-input/50 iso:dark:aria-invalid:ring-destructive/40 iso:[&_svg]:pointer-events-none iso:[&_svg]:shrink-0 iso:[&_svg:not([class*=size-])]:size-4 iso:[&_svg:not([class*=text-])]:text-muted-foreground',
				className
			)}
			{...props}
		>
